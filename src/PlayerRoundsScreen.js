...
function HoleScreen({ hole, total, holeData, onUpdate, onNext, onFinish, onBack, isLast, holesData, currentHoleNum }) {
  ...
  const scoreColor = diff === null ? colors.textTertiary : diff < 0 ? '#16A34A' : diff === 0 ? colors.primary : diff === 1 ? colors.warning : colors.error
        ...
        <View style={s.holeCard}>
          ...
          <View style={s.counter}>
            ...
            <TouchableOpacity onPress={() => { const v = strokes + 1; setLocalStrokes(v); onUpdate({ ...holeData, strokes: v }) }} style={[s.counterBtn, s.counterBtnGreen]}>
              <Text style={[s.counterBtnTxt, { color: colors.textInverse }]}>+</Text>
            </TouchableOpacity>
          </View>
        </View>
          ...
          <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
            {[["left", "← Left"], ["hit", "✓ Hit"], ["right", "Right →"]].map(([val, label]) => (
              <TouchableOpacity key={val} onPress={() => onUpdate({ ...holeData, fairway: holeData.fairway === val ? null : val })} style={[s.toggleBtn, holeData.fairway === val && (val === "hit" ? s.toggleBtnGreen : s.toggleBtnRed)]}>
                <Text style={[s.toggleBtnTxt, holeData.fairway === val && { color: colors.textInverse }]}>{label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
          ...
          <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
            {[["yes", "✓ Oui"], ["no", "✗ Non"]].map(([val, label]) => (
              <TouchableOpacity key={val} onPress={() => onUpdate({ ...holeData, gir: holeData.gir === val ? null : val })} style={[s.toggleBtn, { flex: 1 }, holeData.gir === val && (val === "yes" ? s.toggleBtnGreen : s.toggleBtnRed)]}>
                <Text style={[s.toggleBtnTxt, holeData.gir === val && { color: colors.textInverse }]}>{label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
          ...
          <View style={{ flexDirection: "row", gap: 8, marginTop: 8 }}>
            {[["no", "✓ Non"], ["yes", "✗ Oui"]].map(([val, label]) => (
              <TouchableOpacity key={val} onPress={() => onUpdate({ ...holeData, bunker: holeData.bunker === val ? null : val })} style={[s.toggleBtn, { flex: 1 }, holeData.bunker === val && (val === "no" ? s.toggleBtnGreen : s.toggleBtnRed)]}>
                <Text style={[s.toggleBtnTxt, holeData.bunker === val && { color: colors.textInverse }]}>{label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
    ...
    <View>
      ...
      <View style={{ flexDirection: "row", gap: 6, padding: 16, paddingBottom: 8 }}>
        {["1S","1M","6M","1A","Tout"].map(p => (
          <TouchableOpacity key={p} onPress={() => setPeriod(p)} style={{ flex: 1, paddingVertical: 7, alignItems: "center", borderRadius: 10, borderWidth: 1, borderColor: period === p ? colors.primary : colors.borderStrong, backgroundColor: period === p ? colors.primary : "#fff" }}>
            <Text style={{ fontSize: 12, fontWeight: "700", color: period === p ? "#fff" : colors.textSecondary }}>{p}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <View style={{ flexDirection: "row", gap: 8, paddingHorizontal: 16, paddingBottom: 8 }}>
        {[
          { label: "MEILLEUR", value: best || "—", color: colors.primary },
          { label: "MOYENNE", value: avgScore || "—", color: colors.textPrimary, trend },
          { label: "PUTTS MOY.", value: avgPutts || "—", color: colors.textPrimary },
          { label: "ROUNDS", value: filtered.length, color: colors.textPrimary },
        ].map((stat, i) => (
          <View key={i} style={[s.statCard, i === 0 && { borderTopWidth: 3, borderTopColor: colors.primary }]}>
            ...
            {stat.trend !== undefined && stat.trend !== 0 && (
              <Text style={{ fontSize: 10, fontWeight: "700", color: stat.trend < 0 ? colors.primary : "#EF4444" }}>
        ...
        <View style={s.chartSection}>
          ...
          <View style={{ flexDirection: "row", gap: 4, marginTop: 12, height: 60, alignItems: "flex-end" }}>
            {[
              ...
              { label: "Bogey", count: scoreDist.bogey, color: colors.warning },
              { label: "Dbl", count: scoreDist.double, color: "#EF4444" },
              { label: "+3", count: scoreDist.worse, color: colors.error },
            ].map((item, i) => {
      ...
      <View style={s.chartSection}>
        ...
        {[
          ...
          const color = val ? (stat.lower ? (val <= stat.target ? colors.primary : "#EF4444") : (val >= stat.target ? colors.primary : colors.warning)) : colors.borderStrong
          return (
            <View key={i} style={{ marginBottom: 14 }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
                <Text style={{ fontSize: 13, color: colors.textSecondary, fontWeight: "500" }}>{stat.label}</Text>
                <Text style={{ fontSize: 13, fontWeight: "800", color: val ? color : colors.textTertiary }}>{val || "—"}{val ? " " + stat.unit : ""}</Text>
              </View>
...
export default function PlayerRoundsScreen({ navigation }) {
  ...
  if (screen === "choose") return (
    <SafeAreaView style={s.safe}>
      ...
      <View style={{ padding: 20 }}>
        <Text style={{ fontSize: 14, color: colors.textSecondary, textAlign: "center", marginBottom: 20 }}>Choisis ton mode de saisie</Text>
        ...
        <TouchableOpacity style={[s.modeCard, s.modeCardActive]} onPress={() => setScreen("detailedSetup")}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[s.modeTitle, { color: colors.primary }]}>Detailed mode</Text>
      ...
      <View style={s.tabs}>
        {['rounds', 'stats'].map(t => (
          <TouchableOpacity key={t} style={[s.tab, tab === t && s.tabActive]} onPress={() => setTab(t)}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}><Ionicons name={t === 'rounds' ? 'flag-outline' : 'stats-chart-outline'} size={14} color={tab === t ? colors.primary : colors.textTertiary} /><Text style={[s.tabTxt, tab === t && s.tabTxtActive]}>{t === 'rounds' ? 'Rounds' : 'Stats'}</Text></View>
          </TouchableOpacity>
        ))}
      </View>
          ...
          <>
            {rounds.length === 0 ? (
              <View style={{ padding: 40, alignItems: "center" }}>
                ...
                <Text style={{ fontSize: 16, fontWeight: "700", color: colors.textPrimary, marginBottom: 16 }}>No rounds yet</Text>
                <TouchableOpacity style={s.addBtn} onPress={() => setScreen("choose")}>
              ...
              <View key={r.id} style={s.roundCard}>
                <View style={s.roundCardTop}>
                  ...
                  <TouchableOpacity onPress={() => deleteRound(r.id)} style={{ marginLeft: 12, padding: 6, backgroundColor: colors.errorLight, borderRadius: 8 }}>
                    <Text style={{ fontSize: 13, color: colors.error, fontWeight: '700' }}>✕</Text>
                  </TouchableOpacity>
                </View>
                {(r.putts || r.gir) ? (
                  <View style={{ flexDirection: "row", gap: 12, marginTop: 8 }}>
                    {r.putts ? <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}><Ionicons name="flag-outline" size={12} color={colors.textTertiary} /><Text style={s.roundStat}>{r.putts} putts</Text></View> : null}
...
const s = StyleSheet.create({
  ...
  roundCard: { backgroundColor: colors.surface, borderRadius: 12, margin: 16, marginBottom: 8, padding: 14, borderWidth: 0.5, borderColor: colors.borderStrong, position: 'relative' },
  roundCardTop: { flexDirection: "row", alignItems: "center" },
  roundInfo: { flex: 1 },
  roundCourse: { fontSize: 14, fontWeight: "600", color: colors.textPrimary },
  roundDate: { fontSize: 11, color: colors.textTertiary, marginTop: 2 },
  roundScore: { fontSize: 28, fontWeight: "800", color: colors.primary },
  roundStat: { fontSize: 12, color: colors.textSecondary },
  statCard: { flex: 1, backgroundColor: colors.surface, borderRadius: 12, padding: 10, alignItems: "center", borderWidth: 0.5, borderColor: colors.borderStrong },
  statCardValue: { fontSize: 20, fontWeight: "800" },
  statCardLabel: { fontSize: 9, color: colors.textTertiary, fontWeight: "600", marginTop: 3 },
  chartSection: { backgroundColor: colors.surface, margin: 16, marginTop: 0, marginBottom: 8, borderRadius: 14, padding: 16, borderWidth: 0.5, borderColor: colors.borderStrong },
  chartTitle: { fontSize: 13, fontWeight: "700", color: colors.textPrimary, marginBottom: 8 },
  modalHead: { backgroundColor: colors.surface, flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 16, borderBottomWidth: 0.5, borderBottomColor: colors.borderStrong },
  modalTitle: { fontSize: 17, fontWeight: "700", color: colors.textPrimary },
  backTxt: { fontSize: 16, color: colors.primary, fontWeight: "600" },
  label: { fontSize: 11, fontWeight: "600", color: colors.textSecondary, marginBottom: 6, marginTop: 14 },
  ...
  saveBtnTxt: { color: colors.textInverse, fontSize: 16, fontWeight: "700" },
  modeCard: { backgroundColor: colors.surface, borderRadius: 14, padding: 16, marginBottom: 12, flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderColor: colors.borderStrong },
  modeCardActive: { borderColor: colors.primary, borderWidth: 2 },
  ...
  recommendedBadge: { backgroundColor: colors.primary, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 },
  recommendedTxt: { fontSize: 9, fontWeight: "700", color: colors.textInverse },
  holeHeader: { backgroundColor: colors.primary, padding: 16, paddingTop: 8 },
  holeTitle: { fontSize: 22, fontWeight: "800", color: colors.textInverse },
  ...
  progressDotCurrent: { backgroundColor: colors.surface },
  holeCard: { backgroundColor: colors.surface, borderRadius: 14, padding: 16, borderWidth: 0.5, borderColor: colors.borderStrong },
  cardLabel: { fontSize: 11, fontWeight: "600", color: colors.textTertiary },
  counter: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 },
  counterBtn: { width: 44, height: 44, borderRadius: 22, borderWidth: 1, borderColor: colors.borderStrong, alignItems: "center", justifyContent: "center" },
  counterBtnGreen: { backgroundColor: colors.primary, borderColor: colors.primary },
  counterBtnTxt: { fontSize: 24, color: colors.textSecondary, lineHeight: 28 },
  counterValue: { fontSize: 40, fontWeight: "800", color: colors.textPrimary },
  scoreLabel: { fontSize: 13, fontWeight: "700", marginTop: 2 },
  parBtn: { flex: 1, padding: 10, borderRadius: 10, borderWidth: 1, borderColor: colors.borderStrong, alignItems: "center" },
  parBtnActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  parBtnTxt: { fontSize: 13, fontWeight: "600", color: colors.textSecondary },
  parBtnTxtActive: { color: colors.textInverse },
  toggleBtn: { flex: 1, padding: 10, borderRadius: 10, borderWidth: 1, borderColor: colors.borderStrong, alignItems: "center" },
  toggleBtnGreen: { backgroundColor: colors.primary, borderColor: colors.primary },
  toggleBtnRed: { backgroundColor: "#EF4444", borderColor: "#EF4444" },
  toggleBtnTxt: { fontSize: 13, fontWeight: "600", color: colors.textSecondary },
  holeBtn: { flex: 1, padding: 14, borderRadius: 12, borderWidth: 1, borderColor: colors.borderStrong, alignItems: "center" },
  holeBtnActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  holeBtnTxt: { fontSize: 15, fontWeight: "600", color: colors.textSecondary },
  holeBtnTxtActive: { color: colors.textInverse },
})
